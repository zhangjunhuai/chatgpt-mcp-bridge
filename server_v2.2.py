"""MCP bridge v2.2:工作区沙箱 + argv 执行 + 结构化输出。
...
v2.2:
  - 增加 --default-workdir,未显式传 cwd 时不再默认落到 WRITABLE[0]
  - 增加 --download-dir,download_file 默认下载到独立目录
  - 修复 atomic append 覆盖旧内容的问题
  - 增加 non-destructive create_file 工具,用于安全创建新文件
"""
...
parser.add_argument("--token-file", help="从文件读取 MCP URL token（比命令行参数更适合 systemd）")
...
parser.add_argument("--default-workdir", default=(
    "/media/zhao/add/仿真训练/机制分析/brain_nav_full_handoff_20260909/"
    "current/same_image_diff_history"))
parser.add_argument("--download-dir", default="/home/zhao/chatgpt-downloads")
...
if args.token_file:
    try:
        args.token = Path(args.token_file).expanduser().read_text(encoding="utf-8").strip()
    except OSError as e:
        parser.error(f"cannot read --token-file: {e}")
    if not args.token:
...
DEFAULT_WORKDIR = Path(args.default_workdir).expanduser()
DOWNLOAD_DIR = Path(args.download_dir).expanduser()
try:
    DEFAULT_WORKDIR = DEFAULT_WORKDIR.resolve() if DEFAULT_WORKDIR.exists() else Path(os.path.abspath(DEFAULT_WORKDIR))
    DOWNLOAD_DIR = DOWNLOAD_DIR.resolve() if DOWNLOAD_DIR.exists() else Path(os.path.abspath(DOWNLOAD_DIR))
except OSError:
    pass
...
ANN_CREATE = ToolAnnotations(readOnlyHint=False, destructiveHint=False, openWorldHint=False)
...
def _cwd(cwd: str | None) -> Path | dict:
    target = str(DEFAULT_WORKDIR) if cwd is None else cwd
    p = _read_path(target)
    if isinstance(p, dict):
        return p
    if not p.is_dir():
        return _err(f"cwd is not a directory: {p}")
    return p
...
def create_file(path: str, content: str, atomic: bool = True,
                create_parents: bool = True) -> WriteResult:
    """只创建新的文本文件。目标已存在时拒绝,不会覆盖旧内容。

    这是 non-destructive create 工具;适合生成新的实验脚本、README 和配置文件。
    """
    return write_file(path, content, mode="create", atomic=atomic,
...
def write_file(path: str, content: str, mode: str = "create",
    ...
    atomic=True 时先写同目录临时文件再 replace,避免写一半的损坏文件。
    append+atomic 会先复制旧内容到临时文件,再追加新内容,不会丢失原文件。
    """
    p = _write_path(path)
    if isinstance(p, dict):
        return p
...
def download_file(url: str, filename: str = "", dest_dir: str | None = None) -> DownloadResult:
    """从 http(s) 链接下载文件到工作区(上限 500MB)。

    dest_dir 未指定时使用 --download-dir,避免把下载中转文件写进项目根目录。
    """
    if not url.startswith(("http://", "https://")):
        return _err("only http(s) URLs are allowed")
    base = _write_path(dest_dir or str(DOWNLOAD_DIR))
    if isinstance(base, dict):
        return base
    try:
        base.mkdir(parents=True, exist_ok=True)
    except OSError as e:
        return _err(f"cannot create download directory {base}: {e}")
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "mcp-bridge/2.2"})
